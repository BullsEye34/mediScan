// Use Solidity Version greater than 0.4.22 and less than 0.7.0
...
contract MediScan {
    ...
    function createPatient( string memory name, string memory residenceAddress, uint256  phno, string memory nominees, string memory medicalIssues,  string memory allergies, address patientAddress) public {
    ...
    Patients[patientAddress] = Patient(msg.sender, name, residenceAddress, nominees, phno, medicalIssues, allergies, patientAddress, true);
    
    // Emit the event that the patient was successfully created
    emit patientCreated( msg.sender, name, residenceAddress, nominees, phno, medicalIssues, allergies,patientAddress, true);
